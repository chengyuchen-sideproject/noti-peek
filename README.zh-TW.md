...
> **狀態:可運作 v0.1.0。** 核心攔截 + 本地儲存 + 最小 UI + 依 App 過濾擷取皆已完成。
> 已在 Android 16(API 36)實機建置並驗證;支援 API 26 以上。
...
- **可選擇要擷取哪些 App**:設定頁打開「只擷取選取的 App」並勾選(例如只留 LINE)。
  可**直接從已安裝的 App 挑選**,不必等收到通知才出現。預設是全部擷取。
- **隱私優先設計**:App **未宣告 `INTERNET` 權限** —— 收到的訊息絕不離開這支手機。
  為了在挑選畫面列出已安裝 App,使用 `<queries>` LAUNCHER 宣告(只看得到有桌面圖示的
  App),**並非**敏感的 `QUERY_ALL_PACKAGES` 權限。
...
1. 用 **Android Studio**(Koala / 2024.1+)開啟專案,或直接用命令列建置 —— 已內含
...
5. 選用:點右上角**設定**,選擇要擷取哪些 App。

> `local.properties`(你的 SDK 路徑)不會被 commit —— Android Studio 會建立,或你
> 自己填 `sdk.dir`。Gradle wrapper **有** commit,所以純命令列 `./gradlew` 建置只需
> JDK 17 與 Android SDK(build-tools 34、platform android-34)。
...
│  └─ SettingsStore.kt               擷取過濾設定(SharedPreferences)
├─ service/
│  └─ PeekNotificationListener.kt    核心:攔截 + MessagingStyle 解析
└─ ui/
   ├─ MainActivity.kt                Compose 畫面(首頁 / 對話 / 設定)
   ├─ MessageViewModel.kt
   └─ Theme.kt
```
